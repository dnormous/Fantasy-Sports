...
#Build list of URLs to pull splits data from ESPN (Thank you ESPN!)
  URLs <- c("http://espn.go.com/mlb/stats/pitching/_/count/",
            "http://espn.go.com/mlb/stats/pitching/_/year/2014/count/",
            "http://espn.go.com/mlb/stats/pitching/_/split/61/count/",
            "http://espn.go.com/mlb/stats/pitching/_/year/2014/split/31/count/",
            "http://espn.go.com/mlb/stats/pitching/_/year/2014/split/32/count/",
            "http://espn.go.com/mlb/stats/pitching/_/year/2014/split/33/count/",
            "http://espn.go.com/mlb/stats/pitching/_/year/2014/split/34/count/")
  ...
  for (j in 1:length(URLs)) 
  {
    ...
    #Loop through "Expanded Batting Stats II" table pages (up to 34 of them)
    for (i in 0:34)
    {
      #Set the start of each new table webpage at increments of 1,41,81,etc
      x <- (i*40)+1
      url = paste0(URLs[j],x,URLsEND,"/type/expanded-2")
      ...
      doc <- htmlParse(response, encoding = "UTF-8")
      tableNodes <- getNodeSet(doc, "//table")  
      tbl.e = readHTMLTable(tableNodes[[1]],colClasses = c("character"),stringsAsFactors = FALSE) 
      
      #Construct full table by appending tbl to df
      df.e <- rbind(df.e,tbl.e)
    ...
    colnames(df.e) <- df.e[1,]
    df.e <- df.e[,-1]
    df.e <- df.e[which(df.e[,1] != "PLAYER"),]
    ...
    df <- merge(df,df.d[,c(1,4:length(df.d))],by="PLAYER",all.x=TRUE,all.y=FALSE)
    df <- merge(df,df.e[,c(1,5:length(df.e)-1)],by="PLAYER",all.x=TRUE,all.y=FALSE)
    df <- merge(df,df.t[,c(1,5:length(df.t)-1)],by="PLAYER",all.x=TRUE,all.y=FALSE)
  ...
  #Fix TEAM for players who were traded
    ...
    assign(paste0("P.STATS_",Frames[j]),df)
    
    #Export df as a .csv
    write.csv(df,paste0("C:/Users/Derek/Google Drive/Derek/R/MLB 2015/Stats/Split Stats/P_STATS_",Frames[j],"_",Sys.Date(),".csv"))   
  }
  

  #Create full datatable of all P.STATS
  P.STATS <- P.STATS_CY
  P.STATS <- merge(P.STATS,P.STATS_LY,by="NAME",all.x=TRUE,all.y=FALSE)
  P.STATS <- merge(P.STATS,P.STATS_LW,by="NAME",all.x=TRUE,all.y=FALSE)
  P.STATS <- merge(P.STATS,P.STATS_vL,by="NAME",all.x=TRUE,all.y=FALSE)
  P.STATS <- merge(P.STATS,P.STATS_vR,by="NAME",all.x=TRUE,all.y=FALSE)
  P.STATS <- merge(P.STATS,P.STATS_HM,by="NAME",all.x=TRUE,all.y=FALSE)
  P.STATS <- merge(P.STATS,P.STATS_AW,by="NAME",all.x=TRUE,all.y=FALSE)
  ...
  write.csv(P.STATS,paste0("C:/Users/Derek/Google Drive/Derek/R/MLB 2015/Stats/P_STATS_",Sys.Date(),".csv"))
